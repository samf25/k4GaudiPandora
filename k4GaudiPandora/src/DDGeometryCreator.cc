...
DDGeometryCreator::DDGeometryCreator(const Settings& settings, const pandora::Pandora* const pPandora, const Gaudi::Algorithm* algorithm)
    : m_settings(settings), m_pPandora(*pPandora), m_thisAlg(algorithm) {}
...
pandora::StatusCode DDGeometryCreator::CreateGeometry() const {
  ...
  try {
    ...
    m_thisAlg->debug() << "Creating geometry for detector " << detectorName << endmsg;
    ...
    for (SubDetectorNameMap::const_iterator iter = subDetectorNameMap.begin(), iterEnd = subDetectorNameMap.end();
         iter != iterEnd; ++iter) {
          m_thisAlg->debug() << "Creating geometry for additional subdetector " << iter->first << endmsg;

      PANDORA_RETURN_RESULT_IF(pandora::STATUS_CODE_SUCCESS, !=,
                               PandoraApi::Geometry::SubDetector::Create(m_pPandora, iter->second));
    }
  } catch (std::exception& exception) {
    m_thisAlg->error() << "Failure in marlin pandora geometry creator, exception: " << exception.what() << endmsg;
    throw exception;
  }
...
void DDGeometryCreator::SetMandatorySubDetectorParameters(SubDetectorTypeMap& subDetectorTypeMap) const {
  ...
  try {
    ...
    const dd4hep::rec::LayeredCalorimeterData* coilExtension = getExtension((dd4hep::DetType::COIL), m_thisAlg);
    ...
    m_thisAlg->error() << "Failed to access COIL parameters: " << e.what() << endmsg;
  }
}
...
void DDGeometryCreator::SetAdditionalSubDetectorParameters(SubDetectorNameMap& subDetectorNameMap) const {
  ...
  for (std::vector<dd4hep::DetElement>::const_iterator iter    = theECalOtherDetectors.begin(),
    ...
    try {
      ...
      this->SetDefaultSubDetectorParameters(*const_cast<dd4hep::rec::LayeredCalorimeterData*>(theExtension),
      ...
      m_thisAlg->warning() << "Marlin pandora geometry creator during Other ECal construction: " << exception.what()
                           << endmsg;
    }
  }
  ...
  for (std::vector<dd4hep::DetElement>::const_iterator iter    = theHCalOtherDetectors.begin(),
    ...
    try {
      ...
      m_thisAlg->warning() << "Marlin pandora geometry creator during Other HCal construction: " << exception.what()
                           << endmsg;
    }
  }
  ...
  for (std::vector<dd4hep::DetElement>::const_iterator iter    = theMuonOtherDetectors.begin(),
    ...
    try {
      ...
      m_thisAlg->warning() << "Marlin pandora geometry creator during Other Muon construction: " << exception.what()
                           << endmsg;
    }
  }
}
...
void DDGeometryCreator::SetDefaultSubDetectorParameters(
    const dd4hep::rec::LayeredCalorimeterData& inputParameters, const std::string& subDetectorName,
...
pandora::StatusCode DDGeometryCreator::CreateHCalBarrelBoxGaps() const {
  m_thisAlg->debug() << "Building gaps in detector active material" << endmsg;
  ...
  if ((0 == innerSymmetryOrder) || (2 != outerSymmetryOrder / innerSymmetryOrder)) {
    m_thisAlg->error() << " Detector " << detectorName
  ...
  PANDORA_RETURN_RESULT_IF(
      ...
      this->CreateRegularBoxGaps(innerSymmetryOrder, inner_phi0, innerRadius, outerRadius, -outerZ, outerZ, staveGap));
  ...
  if ((0 == outerPseudoPhi0) || (0.f == cosOuterPseudoPhi0)) {
    m_thisAlg->error() << " Detector " << detectorName << " doesn't conform to expected ILD-specific geometry"
                       << endmsg;
    return pandora::STATUS_CODE_INVALID_PARAMETER;
  }
...
pandora::StatusCode DDGeometryCreator::CreateHCalEndCapBoxGaps() const {
  ...
  PANDORA_RETURN_RESULT_IF(pandora::STATUS_CODE_SUCCESS, !=,
                           this->CreateRegularBoxGaps(innerSymmetryOrder, inner_phi0, innerRadius, outerRadius, innerZ,
                                                      outerZ, staveGap, pandora::CartesianVector(-innerRadius, 0, 0)));

  PANDORA_RETURN_RESULT_IF(pandora::STATUS_CODE_SUCCESS, !=,
                           this->CreateRegularBoxGaps(innerSymmetryOrder, inner_phi0, innerRadius, outerRadius, -outerZ,
                                                      -innerZ, staveGap, pandora::CartesianVector(innerRadius, 0, 0)));
...
pandora::StatusCode DDGeometryCreator::CreateHCalBarrelConcentricGaps() const {
  ...
  PANDORA_RETURN_RESULT_IF(pandora::STATUS_CODE_SUCCESS, !=,
                           PandoraApi::Geometry::ConcentricGap::Create(m_pPandora, gapParameters));
...
pandora::StatusCode DDGeometryCreator::CreateRegularBoxGaps(unsigned int symmetryOrder, float phi0, float innerRadius,
                                                            float outerRadius, float minZ, float maxZ, float gapWidth,
                                                            pandora::CartesianVector vertexOffset) const {
  const pandora::CartesianVector basicGapVertex(pandora::CartesianVector(-0.5f * gapWidth, innerRadius, minZ) +
                                                vertexOffset);
  ...
  for (unsigned int i = 0; i < symmetryOrder; ++i) {
    ...
    gapParameters.m_vertex = pandora::CartesianVector(cosPhi * basicGapVertex.GetX() + sinPhi * basicGapVertex.GetY(),
                                                      -sinPhi * basicGapVertex.GetX() + cosPhi * basicGapVertex.GetY(),
                                                      basicGapVertex.GetZ());
    gapParameters.m_side1 =
        pandora::CartesianVector(cosPhi * basicSide1.GetX() + sinPhi * basicSide1.GetY(),
                                 -sinPhi * basicSide1.GetX() + cosPhi * basicSide1.GetY(), basicSide1.GetZ());
    gapParameters.m_side2 =
        pandora::CartesianVector(cosPhi * basicSide2.GetX() + sinPhi * basicSide2.GetY(),
                                 -sinPhi * basicSide2.GetX() + cosPhi * basicSide2.GetY(), basicSide2.GetZ());
    gapParameters.m_side3 =
        pandora::CartesianVector(cosPhi * basicSide3.GetX() + sinPhi * basicSide3.GetY(),
                                 -sinPhi * basicSide3.GetX() + cosPhi * basicSide3.GetY(), basicSide3.GetZ());

    PANDORA_RETURN_RESULT_IF(pandora::STATUS_CODE_SUCCESS, !=,
                             PandoraApi::Geometry::BoxGap::Create(m_pPandora, gapParameters));
  }
